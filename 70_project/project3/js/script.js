$(function () {
    ...
    // scrollTop
    // 헤더
    const headerSt = $('header');
    const slideSt = $('#slide');

    /* // 윈도우에 스크롤 이벤트가 발생하면 함수 실행
    window.onscroll(function () {
        // 스크롤바를 스크롤한 양을 st에 저장
        let st = document.documentElement.scrollTop; // 이게 잘못됨
        console.log(st);
        // let stVal = 600;
        if (st >= 50) {
            headerSt.css({ top : '0px'});
           
        console.log(123);

        } else {
            headerSt.css({ top : '-54px'});
        }
    ...
    window.onscroll = function () {
        ...
        if (scr_e >= 50) {
            document.querySelector("header").style.top = "-54px";
        } else {
