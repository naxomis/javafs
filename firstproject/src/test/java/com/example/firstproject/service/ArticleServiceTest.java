...
//        }
    ...
    void show_실패() {
        // 예상
        Long id = -1L;
        // 테스트의 사용할 id값으로 -1L 이 사용 - 이값은 존재하지 않는 article id 값으로 설정
        Article expected = null;    // 예상되는 결과로 null 이설정
        // 실제
        // articleService.show(id); 로 조회된 article 이 존재하지 않아 null 을 반환
        Article article = articleService.show(id);
        // 비교
        assertEquals(expected, article);
        // 기재하는 값도 null 이고 articleService 에서 해당 id 로 조회도니 Article 이 존재
